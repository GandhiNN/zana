...
impl Glue {
    ...
    pub async fn list_tables(&self, database: &str) -> Result<Vec<GlueTable>, Error> {
        let mut list_tables = self
            .client
            .get_tables()
        ...
        while let Some(list_tables_output) = list_tables.next().await {
            match list_tables_output {
                Ok(list_tables) => {
                    ...
                    for table in tables {
                        let table_params =
                            table.storage_descriptor().unwrap().parameters().unwrap();
                        glue_tables.push(GlueTable {
                            ...
                            table_record_count: table_params
                                ...
                                .unwrap()
                                .to_string(),
                            table_avg_record_size: table_params
                                .get("recordCount")
                                .unwrap()
                                .to_string(),
    ...
    pub async fn list_jobs(&self) -> Result<Vec<GlueJobNames>, Error> {
        let mut list_jobs = self.client.list_jobs().into_paginator().send();
    ...
    pub async fn get_job_runs(&self, job_name: &str) -> Result<Vec<GlueJobRun>, Error> {
    ...
    pub async fn get_job_bookmark(&self, job_name: &str) -> Result<(), Error> {
        let job_bookmark = self
            .client
            .get_job_bookmark()
            .job_name(job_name)
            .send()
    ...
    pub async fn list_databases(&self) -> Result<Vec<GlueDatabase>, Error> {
        let mut list_databases = self.client.get_databases().into_paginator().send();
